...
**Status: v1.2 (ratified)** · normative chapter · *amended in v1.2, v1.3, v1.4, v1.5, v1.8, v1.10, v1.15, v1.16, v1.17, v1.20, v1.23, v1.25, v1.26, v1.32, v1.34, v1.35 and v1.36 — see [CHANGELOG.md](CHANGELOG.md)*
...
| `kind` | enum `counter\|gauge\|text\|bool\|histogram` | no (v1.32; `histogram` v1.36) | what the leaf value *is*, so a judge can say when it is not: **counter** — a non-negative number that never decreases within one origin's series except across a producer restart (visible as the origin's `alive` token cycling, [04-planes.md §5](04-planes.md)); **gauge** — any number; **text** — a string; **bool** — a boolean; **histogram** (v1.36) — a fixed-bucket distribution (the Prometheus classic / OTLP explicit-bucket shape) over the boundaries the entry's `buckets` declares, whose observed boundaries MUST equal the declared ones; its payload shape is the application profile's, not this chapter's. Absent = unchecked, and a judge treats it as *not asked* ([13 §3](13-observer-conformance.md)). A payload MAY self-describe as an internally tagged value object `{"type": "<kind>", "value": …}` (tag `boolean` for `bool`; the reference application's `TelemetryValue` has that shape, [11 §4](11-zensight-profile.md)) — when it does, the tag MUST agree with `kind`, and the application's typed builders SHOULD refuse the disagreeing variant at build time |
| `buckets` | array of numbers | **iff** `kind = "histogram"` (v1.36) | the histogram's upper bounds, strictly ascending and finite, `+Inf` implicit (never written). Required with `kind = "histogram"` and refused on any other entry: two producers of one subject are comparable only if their boundaries are, and an optional list would reproduce the problem a `histogram` kind exists to solve. A payload that states its boundaries MUST state these |
| `semantic` | enum `temperature\|power\|bytes\|duration\|ratio\|count\|identity\|state` | no (v1.36) | a presentation hint for the case `kind` and `unit` leave ambiguous — a `1`-unit gauge that is a **ratio** (0–1) versus a **count**, a `By` gauge that is **bytes** of capacity, a text subject that is an **identity** (a hostname, a serial: never truncated) versus a **state** (an enum: a chip). Closed vocabulary; an unknown value is a lint. Absent changes nothing — consumers derive the obvious cases from `unit` — and no judge reads it: it says how to *show* a value, never what the value must be |
