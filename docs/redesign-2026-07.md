...
| Daemon | **No** (Zenoh discovery is fast; ros2's daemon solves a DDS problem we don't have). On-disk slice cache `~/.cache/zenkey-explorer/<context>/slices/` gives instant dynamic completions (shipped in #54). *Amended when #227 shipped `zenctl watchdog`: the noun this row rejects is a **hidden, auto-started, shared-state background server** whose job is caching discovery — the ros2 shape. A **foreground observer** — explicitly launched, single-purpose, one process per invocation, no shared state, no effect on any other invocation — is not that, and was never what this row rejected. `watchdog` is the second kind; the decision stands against the first.* *Amended again when #388 shipped `zenwatch`: it **is** a daemon — it runs until stopped — and it is still not the rejected noun. It caches no discovery, shares nothing with any explorer, and its only state is its own notification ledger, bounded and reported; it publishes that state the way a producer does (`health`, `firing/{rule_id}`, `doctor`), never through a side channel another tool reads. The decision stands against the first kind; two of the second now exist.* *Amended a third time when #228 shipped `zenctl export`: a foreground observer serving `/metrics` — explicitly launched, single-purpose, one process per invocation, shares nothing, caches no discovery, serves nothing another zenctl reads. `export` is the third of the second kind.* |
